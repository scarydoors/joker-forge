...
const generateJokerCode = (
  ...
  if (joker.rules && joker.rules.length > 0) {
    ...
    if (pokerHandRules.length > 0) {
      // Generate specialized poker hand code
      const pokerHandDesc = generatePokerHandDescription(joker, joker.rules);

      // Override description if we got one
      if (pokerHandDesc.length > 0) {
        // Need to replace the description in jokerCode
        const descStart = jokerCode.indexOf("loc_txt = {");
        const descEnd = jokerCode.indexOf("    },", descStart);

        const newDesc = `loc_txt = {
        ['name'] = '${joker.name}',
        ['text'] = {
            ${pokerHandDesc
              .map((line, i) => `[${i + 1}] = '${line}'`)
        ...
        jokerCode =
          jokerCode.substring(0, descStart) +
          newDesc +
          jokerCode.substring(descEnd + 6);
      }
    ...
    // No rules, use default generators
    locVarsCode = generateBasicLocVarsFunction(joker);
    calculateCode = generateBasicCalculateFunction(joker);
  }
