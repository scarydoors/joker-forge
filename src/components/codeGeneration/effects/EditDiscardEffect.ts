...
export const generateEditDiscardReturn = (effect: Effect): EffectReturn => {
  ...
  const configVarName = getEffectVariableName(effect.id, "discards");
  let statement = "";
  ...
  switch (operation) {
    case "add": {
      const addMessage = customMessage
        ? `"${customMessage}"`
        : `"+"..tostring(card.ability.extra.${configVarName}).." Discard"`;
      statement = `func = function()
                card_eval_status_text(context.blueprint_card or card, 'extra', nil, nil, nil, {message = ${addMessage}, colour = G.C.ORANGE})
                G.GAME.current_round.discards_left = G.GAME.current_round.discards_left + card.ability.extra.${configVarName}
                return true
            end`;
      break;
    }
    case "subtract": {
      ...
      statement = `func = function()
                ...
                G.GAME.current_round.discards_left = math.max(0, G.GAME.current_round.discards_left - card.ability.extra.${configVarName})
                return true
            end`;
      break;
    }
    case "set": {
      const setMessage = customMessage
        ? `"${customMessage}"`
        : `"Set to "..tostring(card.ability.extra.${configVarName}).." Discards"`;
      statement = `func = function()
                card_eval_status_text(context.blueprint_card or card, 'extra', nil, nil, nil, {message = ${setMessage}, colour = G.C.BLUE})
                G.GAME.current_round.discards_left = card.ability.extra.${configVarName}
                return true
            end`;
      break;
    }
