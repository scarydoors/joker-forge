...
export const exportJokersAsMod = async (
  ...
  try {
    ...
    zip.file(metadata.main_file, generateMainLua(jokers));
...
-- Helper function for card destruction
...
export const exportSingleJoker = (joker: JokerData): void => {
  try {
    const jokerCode = generateSingleJokerLua(joker);
...
const generateSingleJokerLua = (joker: JokerData): string => {
  const passiveEffects = processPassiveEffects(joker);
  globalEffectVariableMapping = {};

  const hasDeleteEffects = joker.rules?.some((rule) =>
    [
      ...(rule.effects || []),
...
`;
  ...
  if (hasDeleteEffects) {
    output += `-- Helper function for card destruction
local function contains(table_, value)
    for _, v in pairs(table_) do
        if v == value then
            return true
